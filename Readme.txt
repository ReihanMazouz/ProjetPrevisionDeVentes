...
Le repository gitHub est constitué d'un dossier "ProjectDataAnalysisPython" où se situe l'ensemble de notre travail. Il est constitué de l'application Flask ( son fonctionnement est détaillé dans la présentation power point), d'un modèle de prédiction de vente enregistré ici mais qui a peu d'importance, du fichier code python jupyter notebook avec l'implémentation de nos modèles et sa page html associée. Enfin il y a le power point faisant office de petite documentation et le dataset que nous avons utilisé. 
